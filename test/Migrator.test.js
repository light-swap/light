...
contract('Migrator', ([alice, bob, dev, minter]) => {
    beforeEach(async () => {
        ...
        this.lightMain = await LightMain.new(this.lightToken.address, 1, '1000', '0', '100000', '25000', '420000000000000000000000', { from: alice });
        this.migrator = await Migrator.new(this.lightMain.address, this.factory1.address, this.factory1.address, this.factory2.address, '0');
        await this.lightToken.transferOwnership(this.lightMain.address, { from: alice });
        await this.lightMain.add('100', this.lp1.address, true, false, { from: alice });
    });
    ...
    it('should do the migration successfully', async () => {
        ...
        await this.lp1.sync();
        await this.lp1.approve(this.lightMain.address, '100000000000', { from: minter });
        await this.lightMain.deposit('0', '2000000', { from: minter });
        assert.equal((await this.lp1.balanceOf(this.lightMain.address)).valueOf(), '2000000');
        await expectRevert(this.lightMain.migrate(0, 0, 0), 'migrate: no migrator');
        await this.lightMain.setMigrator(this.migrator.address, { from: alice });
        await expectRevert(this.lightMain.migrate(0, 0, 0), 'migrate: bad');
        await this.factory2.setMigrator(this.migrator.address, { from: alice });
        await this.lightMain.migrate(0, 0, 0);
        assert.equal((await this.lp1.balanceOf(this.lightMain.address)).valueOf(), '0');
        assert.equal((await this.lp2.balanceOf(this.lightMain.address)).valueOf(), '2000000');
        await this.lightMain.withdraw('0', '2000000', { from: minter });
